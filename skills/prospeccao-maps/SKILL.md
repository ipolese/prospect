...
description: Prospectar clientes no Google Maps, buscar negócios bem avaliados com sites ruins, qualificar leads e montar a base. Usar ao prospectar, buscar clientes, achar leads ou procurar clientes com site ruim.
---
...
## Fluxo (via navegador integrado do Codex)
...
Quando houver conector Google Drive/Sheets, criar uma planilha `Leads Prospector — [nicho] [cidade]`, incluindo qualificados e descartados. Sem conector, criar CSV local e continuar; planilha externa não é requisito bloqueante.
...
Status possíveis: `novo`, `redesenhado`, `publicado-local`, `publicado`, `proposta`. Quando um status mudar, atualizar banco, cópia local e dashboard. Regenerar a planilha externa somente quando o conector estiver disponível. Nunca sobrescrever leads antigos.
